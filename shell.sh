...
116 IFS 是一种 set 变量，当 shell 处理"命令替换"和"参数替换"时，shell 根据 IFS 的值，默认是 space, tab, newline 来拆解读入的变量，然后对特殊字符进行处理，最后重新组合赋值给该变量

117 [-n  参数]  可以用来判断该参数是否已被赋值 没赋值，返回flase //?????

118 [ -n str1 ]　　　　　　 当串的长度大于0时为真(串非空) 
...
119 
    ...
    3.通过source或“.”加载执行过的脚本，由于是在当前Shell中执行脚本，因此在脚本结束之后，脚本中的变量(包括函数)值在当前Shell中依然存在，
      而sh和bash执行脚本都会启动新的子Shell执行，执行完后退回到父Shell。因此，变量(包括函数)值等无法保留。在进行Shell脚本开发时，
      如果脚本中有引用或执行其他脚本的内容或配置文件的需求时，最好用“.”或source先加载该脚本或配置文件，处理完之后，再将它们加载到脚本的下面，
      就可以调用source加载的脚本及配置文件中的变量及函数等内容了。

120 可以通过非0的数字定位参数，0为命令本身，1，2，3，4，5，，，为命令后面的参数.  ${@:2}  //取2及其后面的所有的参数.

121 shell中的=~的简单用法
    其中 ~ 其实是对后面的正则表达式表示匹配的意思，如果匹配就输出1， 不匹配就输出0
    [[ $test =~ ^[0-9]+ ]] && echo 1 || echo 0

    if [[ $exclude_name =~ (^|[[:space:]])$name($|[[:space:]]) ]]

122 ^    以^后面字符开头的字符进行匹配

123 wps &    子进程执行命令. 

124  在"set -e"之后出现的代码，一旦出现了返回值非零，整个脚本就会立即退出，那么就可以避免一些脚本的危险操作